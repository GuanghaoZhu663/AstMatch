...
def var_all_case_LA(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, nms=0):
   
    with open('.../data_split/LA/test.list', 'r') as f:
        image_list = f.readlines()
    image_list = [".../data_split/LA/2018LA_Seg_Training Set/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
...
def var_all_case_Pancreas(model, num_classes, patch_size=(96, 96, 96), stride_xy=18, stride_z=4, nms=0):
    with open('.../data_split/Pancreas/test.list', 'r') as f:
        image_list = f.readlines()
    image_list = [".../data_split/Pancreas/data/" + item.replace('\n', '') for item in image_list]
    loader = tqdm(image_list)
    total_dice = 0.0
    for image_path in loader:
        h5f = h5py.File(image_path, 'r')
