...
from networks.VNet import s4GAN_discriminator_ECSA_3d_LA
from networks.net_factory import net_factory
from utils.utils import context_mask
...
def find_good_bad_maps(D1_out_weak, outputs_weak, outputs_strong_1, outputs_strong_2, weak_inputs, strong_inputs, label, args):
    ...
    if bad_count >0:
        ...
        mask_ratio = 0.3 ** (1 / 3)
        img_mask, _ = context_mask(labeled_img, mask_ratio)
...
def self_train(args, self_snapshot_path):
    ...
    model_D = s4GAN_discriminator_ECSA_3d_LA(num_classes=args.num_classes).cuda()
    model_D2 = s4GAN_discriminator_ECSA_3d_LA(num_classes=args.num_classes).cuda()
    ...
    for epoch in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            for oo in range(unlabeled_volume_batch.shape[0]):
                ...
                Cutout_opt_1 = losses.Cutout(5, int(60 * cut_ratio_1))
                ...
                Cutout_opt_2 = losses.Cutout(5, int(60 * cut_ratio_1))
            ...
            if iter_num % 200 == 0:
                model.eval()
                dice_sample = test_3d_patch.var_all_case_LA(model, num_classes=args.num_classes, patch_size=args.patch_size, stride_xy=18, stride_z=4, nms=1)
...
if __name__ == "__main__":
    ...
    self_snapshot_path = "./model/LA_{}_{}_labeled/self_train".format(args.exp, args.labelnum)
    ...
    shutil.copy('../code/LA_AstMatch_train.py', self_snapshot_path)
